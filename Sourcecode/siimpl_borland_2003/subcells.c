...
void cr_subcells( void)
{
   ...
   struct double3D temp3D, ea12, ea23, ea31, a12, a23, a31, nmega, r0SC;
   ...
   // and the SCsize parametrer
	nSC.x = ceil( norm3D(&(UCbase.a1))/ SCsize);
	nSC.y = ceil( norm3D(&(UCbase.a2))/ SCsize);
	nSC.z = ceil( norm3D(&(UCbase.a3))/ SCsize);
   ...
   // the atom data (SCstruct) are stored in MCatom;
	...
	nmega.x = ceil(rmax/dot3D(&(UCbase.a1), &ea23));
	nmega.y = ceil(rmax/dot3D(&(UCbase.a2), &ea31));
	nmega.z = ceil(rmax/dot3D(&(UCbase.a3), &ea12));
